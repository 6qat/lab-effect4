...
2. **The Bun engine's connect is restructured from `Effect.tryPromise` to `Effect.callback`**, mirroring the Node.js engine: a `cancelled` flag routes late promise settlement to an immediate `socket.end()` teardown instead of handing over a handle nobody will release, and the cleanup return sets the flag on interruption. Since `Bun.connect` exposes no abort signal, an in-flight connect cannot be torn down before it settles; the flag guarantees settlement never leaks the socket.
...
5. **Hygiene**: gated-schedule taps complete the gate `Deferred` directly; the TLS fixture uses `os.tmpdir()`; the retry-recovery test is bounded by an explicit `Effect.timeout("5 seconds")`; the dead `basePort` option is removed from the suite and its callers.
...
- Engine adapters now tear down interrupted connect attempts by themselves, promptly and with the correct exit, instead of relying on the ambient scope's cascade — completing the scope-ownership contract ADR 0006 stated.
- The Bun engine closes its remaining leak window: connects interrupted mid-flight (common against production networks) can no longer produce ownerless connected sockets.
...
- The interrupt-mid-handshake guard-rail test (silent TLS server, prompt bounded interrupt, no defects) passes both before and after these changes: the ambient-scope cascade already cleaned up during unwinding once ADR 0006's parent-linkage landed. It locks the contract going forward rather than proving a regression fix; the Bun late-resolution leak, by contrast, is real but not deterministically reproducible black-box (the connect must resolve after the interrupt), so it is covered by the restructure plus code review rather than a red test.
